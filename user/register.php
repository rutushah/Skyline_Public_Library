        ...
        if(!$emailErr && !$strong_password_Err && !$phoneErr && !$profileError && !$residenceProofError && !$passwordErr && !$birthPlaceError && !$petNameError && !$motherNameError && !$lnameError && !$fnameError && !$addressError)
        {
            if ($user_matched > 0) 
            {
                ...
                if(in_array($file_ext_check,$valid_file_ext) &&in_array($file_ext_check_residenceProof, $valid_file_ext) ){
                    ...
                    "INSERT INTO users
                    (`first_name`, `last_name`, `email_id`, `password`, `contact_number`, `address`, `registration_date`,`pet_name`, `mother_name`, `birth_place`,
                    `image`,`residence_proof`,`user_role`,`account_status`) 
                    VALUES ('$fname','$lname','$email','$hashed_password','$phone','$address','$registration_date','$pet_name','$mother_name',
                     ...
                     <div class="from-wrapper">
                      <form action="" method="post"  name="registration_form" autocomplete="off" enctype = "multipart/form-data">
                        ...
                        <div class="mb-3 row d-flex ms-5">
                           ...
                           <div class="col-6 ">
                            <input type="submit" class="btn btn-primary px-4 py-2 rounded-4  bg-success" name="register" value="Back"></div>
                        </div>
                        <div class="mb-3">Already have account?&nbsp;&nbsp;<a class="text-success" href="#"><u>Login</u></a></div>
                      </form>
                     </div>
                  </div>
               </div>
